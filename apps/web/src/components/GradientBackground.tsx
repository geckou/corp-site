...
export const GradientBackground = forwardRef<GradientBackgroundRef, GradientBackgroundProps>(
  ({ scrollPercentage }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      resetScroll: () => { lastScrollPercentage.current = 0 },
    }))

    const calcOpacity = useCallback((index: number): number => {
    ...
    const conversionGradientColor = (gradients: { color: string; gradientChangePoint: string }[]): string => {
        ...
        {backgroundGradients.map((gradient, index) => (
          <div
            ...
            style={{
              ...
              opacity        : backgroundGradients.length === index + 1 ? 10 : calcOpacity(index),
