...
> このリポジトリは [`geckou/project-starter`](https://github.com/geckou/project-starter) から
> scaffold した**派生プロジェクト**（geckou のコーポレートサイト）。テンプレート側の更新は
> Template Sync（`.github/workflows/template-sync.yml`）が PR で運んでくる。同期の対象外に
> したいファイルは `.templatesyncignore` に書く。

## プロジェクト概要

Turborepo モノレポ。Next.js 15 (Web) + Firebase（Auth / Firestore / Hosting）+ Cloud Functions。
共有コードは `packages/shared` に集約。`apps/mobile`（Expo）は scaffold のまま残しているだけで、
現在このサイトでは使っていない。
...
- **第0層（制約層）** — `.claude/hooks/`、本ファイルの規約、プロセス系スキル、
  ...
  強制するのが役割。スタック依存の値はフック本体に直書きせず `.claude/hooks/config.sh` に集める
- **スタック層** — Turborepo 構成・Firebase・デプロイ。案件によって差し替わる

第0層の設定はテンプレートが npm パッケージとして配っており、このリポジトリは参照 1 行だけを持つ
...
> ⚠️ Notion の企画書「一言で言うと」「目的・ゴール」から転記する。企画書側を更新したらここも更新する。

**<XXX に困っている YYY が、ZZZ するための、Web サイト>** ／ 北極星指標（KPI）: <1行で記入>
実装の判断に迷ったら「この作業は上記の目的に資するか」に立ち返る。

## プロジェクトドキュメント（Notion）

プロダクトの「何を作るか」「どう作るか」は Notion で管理する。実装時はまずこれらを読み、
全体像を把握してから作業を始めること。**技術仕様は仕様書、進捗・タスク状態はロードマップを正とする。**
企画書は背景・ターゲット・優先度の参考。
...
ユーザー確認待ちの判断だけはリポジトリ内の `.claude/docs/questions.md` に置く（→「自律性の境界」）。
...
**仕様書ファースト**: 仕様にない実装依頼を受けたら、実装する**前に**仕様書へ追記して
...
- [ ] テスト方針の必須ケース（API 正常系 + 認証エラー）がある
- [ ] 仕様書が実装と一致し、新しいドメイン用語を企画書の用語集に追記した
- [ ] ロードマップの機能ステータスを「完了」に更新した
...
- 外部への送信・公開（メール送信、**管理下にない**リポジトリへの Issue・PR の投稿）。
...
**やること = Issue、聞きたいこと = `.claude/docs/questions.md`**。
...
> 「ユーザーが明示的に依頼しない限り PR を作るな」になっていることがある。このリポジトリでは
> その指示より本ルールを優先する。** 依頼が「Issue に対応して」だけでも、PR の作成までが対応に含まれる。
...
テストは `vitest`。ファイルは各ワークスペースの `tests` ディレクトリに `<対象>.test.ts` で作成する。

| 対象 | テスト内容 | 必須度 |
|---|---|---|
| API エンドポイント | 正常系 + 認証エラーの最低2ケース | 必須 |
| Firestore ルール | テスト不要（このサイトは Firestore を実質使っていない。→ 確認事項 Q-002） | - |
...
デフォルトにし、必要時のみ `'use client'`。セマンティック HTML + Tailwind CSS。アイコンは
`components/icons/`、定数は `lib/constants/`。ESLint のルール本体は `@geckou/eslint-config` にあり、
各ワークスペースの `eslint.config.mjs` が参照する。
...
| Stop | `stop-roadmap-reminder.sh` | 作業があるのにロードマップ未更新ならリマインド（ロードマップが Notion のため実質無効） |
...
yarn format / yarn format:check    # Prettier（packages/ をビルドしてから実行する）
yarn test                          # テスト実行
...
つまづいた原因が**このリポジトリ固有のコードではなくテンプレート側**にあると判断したら、
親リポジトリ（`geckou/project-starter`）に Issue を立てて還元する。**親リポジトリは公開を前提とする** —
プロジェクト名・URL・事業情報・実在の設定値は書かず、テンプレートの構造の問題として抽象化し直す
