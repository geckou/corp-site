...
```
1. ロードマップ（Notion。→ CLAUDE.md「プロジェクトドキュメント」）で対象機能を「実装中」に更新
2. 企画書（Notion）の用語集でドメイン用語を確認（→ 変数名・コレクション名に使う）
3. 仕様書（Notion）のデータモデルを確認 → /new-collection でスキーマ・型・API・テストを生成
...
1. 実装する**前に**仕様書（必要なら企画書のコア機能一覧・ロードマップの機能ステータス）に追記し、ユーザーの承認を得る
2. 承認後に実装を開始する
